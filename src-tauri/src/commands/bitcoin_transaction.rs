use crate::models::bitcoin_transaction::{BitcoinTransaction, CreateBitcoinTransactionRequest, UpdateBitcoinTransactionRequest, PaginatedBitcoinTransactions, TransactionType};
...
pub async fn get_bitcoin_transactions(
    ...
    let total_count: i64 = sqlx::query_scalar(
        "SELECT COUNT(*) FROM bitcoin_transactions"
    )
    ...
    let rows = sqlx::query(
        "SELECT id, type, amount_sats, fiat_amount_cents, fee_sats, fee_fiat_cents, memo, timestamp, created_at FROM bitcoin_transactions ORDER BY timestamp DESC LIMIT ? OFFSET ?"
    )
    ...
    for row in rows {
        let transaction = BitcoinTransaction {
            id: row.get("id"),
            r#type: row.get::<String, _>("type").parse().map_err(|e| format!("Invalid transaction type: {}", e))?,
    ...
    println!("Retrieved {} bitcoin transactions (page {} of {}, has_more: {})", result.transactions.len(), page, total_pages, has_more);
    Ok(result)
}
...
pub async fn update_bitcoin_transaction(
    ...
    sqlx::query(
        "UPDATE bitcoin_transactions SET type = ?, amount_sats = ?, fiat_amount_cents = ?, fee_sats = ?, fee_fiat_cents = ?, memo = ?, timestamp = ? WHERE id = ?"
    )
    .bind(request.r#type.to_string())
    .bind(request.amount_sats)
    .bind(request.fiat_amount_cents)
    .bind(request.fee_sats)
    ...
    let row = sqlx::query(
        "SELECT id, type, amount_sats, fiat_amount_cents, fee_sats, fee_fiat_cents, memo, timestamp, created_at FROM bitcoin_transactions WHERE id = ?"
    )
    .bind(&id)
    .fetch_one(pool.inner())
    .await
